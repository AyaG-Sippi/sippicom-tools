<#
.SYNOPSIS
    SIPPICOM Workstation AutoDeploy Cloud Launcher
.DESCRIPTION
    irm https://raw.githubusercontent.com/AyaG-Sippi/sippicom-tools/main/tools/autodeploy/AutoDeploy.ps1 | iex
#>

$BASE_URL = "https://raw.githubusercontent.com/AyaG-Sippi/sippicom-tools/main"
$exeUrl = "$BASE_URL/bin/SippicomAutoDeploy.exe"
$localExe = [System.IO.Path]::Combine($env:TEMP, "SippicomAutoDeploy.exe")

Write-Host "--> Launching SIPPICOM Workstation AutoDeploy from Cloud..." -ForegroundColor Cyan
...
try {
    ...
    Write-Host "! GUI fetch failed. Running in-memory Winget installer..." -ForegroundColor Yellow
